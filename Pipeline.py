...
#
# Output files:
#   ./outputs/fdd_out/results_all.csv
#   ./outputs/fdd_out/feature_importance_all.csv
#   ./outputs/fdd_out/confmat_all.csv
#   ./outputs/fdd_out/model_stats_all.csv
#   ./outputs/fdd_out/model_table_rows.tex
# ============================================================
...
FDD_OUT_DIR = OUTPUT_DIR / "fdd_out"
PFI_OUT_DIR = OUTPUT_DIR / "pfi_out"
...
# Global feature importance configuration
...
def stratified_split_keep_singletons_in_train(X, y, test_size=0.2, seed=42, tag=""):
    ...
    if len(idx_rest) < 5:
        if tag:
            print(
                f"Warning [{tag}]: Too few non-singleton samples after removing singleton classes. "
                f"Using all rows for training."
            )
        return X, X[:0], y, y[:0]
    ...
    if cnts2.min() < 2:
        if tag:
            print(
                ...
                f"Falling back to non-stratified random split."
            )
...
def show_feature_importance_signed(
    feat_names,
    mean_imp,
    std_imp,
    title,
...
def add_feature_importance_rows(
    src,
    building,
    label_mode,
    tag_base,
...
def add_model_stats_row(
    ...
    MODEL_STATS_ROWS.append({
        ...
        "param_count": (
            int(param_count)
            if param_count is not None and not pd.isna(param_count)
            else np.nan
        ),
        "node_count": (
            int(node_count)
            if node_count is not None and not pd.isna(node_count)
            else np.nan
        ),
        "leaf_count": (
            int(leaf_count)
            if leaf_count is not None and not pd.isna(leaf_count)
            else np.nan
        ),
        "tree_count": (
            int(tree_count)
            if tree_count is not None and not pd.isna(tree_count)
            else np.nan
        ),
        "model_size_bytes": (
            int(model_size_bytes)
            if model_size_bytes is not None and not pd.isna(model_size_bytes)
            else np.nan
        ),
        "model_size_human": (
            bytes_to_human(model_size_bytes)
            if model_size_bytes is not None and not pd.isna(model_size_bytes)
            else ""
        ),
...
def collect_model_stats_for_building(src, building, label_mode, tag_base, trained, n_features, n_classes):
    if "LinearSVM" in trained:
        ...
        add_model_stats_row(
            ...
            notes=(
                f"coef_shape={tuple(m.coef_.shape)}, "
                f"intercept_shape={tuple(m.intercept_.shape)}, "
    ...
    if "XGBoost" in trained:
        ...
        add_model_stats_row(
            ...
            family="Tabular",
            architecture=(
                f"300 boosted trees; maximum depth {XGB_MAX_DEPTH}; "
                f"learning rate {XGB_LR}; row/column subsampling 0.85."
            ),
            hidden_col="--",
            node_count=nodes,
            leaf_count=leaves,
    ...
    if "LSTM" in trained:
        ...
        add_model_stats_row(
            ...
            notes=(
                f"input_dim={m.lstm.input_size}, hidden={m.lstm.hidden_size}, "
                f"layers={m.lstm.num_layers}, n_classes={n_classes}"
            )
        )
    ...
    if "CNN-LSTM" in trained:
        ...
        add_model_stats_row(
            ...
            table_size_text=f"{p:,} params",
            notes=(
                f"input_dim={m.cnn.in_channels}, conv_channels={m.cnn.out_channels}, "
                f"hidden={m.lstm.hidden_size}, n_classes={n_classes}"
            )
        )
    ...
    if "Informer" in trained:
        ...
        add_model_stats_row(
            ...
            family="Sequence",
            architecture=(
                f"Input projection to d={d_model} + {num_layers}×Transformer encoder "
                f"({nhead} heads) + linear classifier."
            ),
            ...
            table_size_text=f"{p:,} params",
            notes=(
                f"input_dim={m.in_proj.in_features}, d_model={d_model}, "
                f"nhead={nhead}, layers={num_layers}, ff_dim={ff_dim}, "
...
def degrade_noise(X, lvl):
    return X + np.random.normal(
        0,
        np.std(X, axis=0, keepdims=True) * lvl,
        size=X.shape
    )
...
def sage_importance_tabular(
    ...
    for p in range(n_perms):
        ...
        Xmasked = _mask_all_features_tabular(
            Xs,
            X_train,
            feat_idx,
            rng,
...
def sage_importance_sequence(
    ...
    for p in range(n_perms):
        ...
        Xmasked = _mask_all_features_sequence(
            Xs,
            X_train_tab,
            feat_idx,
            rng,
...
def load_dataset_lbnl():
    ...
    for b, p in file_map.items():
        ...
        log_kv(
            "Faults(raw)",
            list(meta["inv_fault_map"].values())[:10]
            + (["..."] if len(meta["inv_fault_map"]) > 10 else [])
        )
...
def load_dataset_wang():
    ...
    log_kv("Wang raw folder", ROOT)

    file_map = {
        "auditorium": ROOT / "auditorium_scientific_data.csv",
        "office": ROOT / "office_scientific_data.csv",
        "hospital": ROOT / "hosptial_scientific_data.csv",
    }
    ...
    for name, path in file_map.items():
        if path.exists():
            ...
            log_kv(f"Skip {name}", f"missing file: {path}")
...
def run_one_building_train_once(item):
    ...
    if SPLIT_MODE == "time":
        ...
        raise ValueError(
            ...
            "'time', 'stratified', 'stratified_time', 'episode_stratified_time'"
        )
    ...
    if USE_XGBOOST:
        ...
        if K < 2:
            log_kv("XGBoost", f"skip because only {K} class in train after split/cap")
        else:
    ...
    for manip_name, lvls in levels.items():
        for lvl in lvls:
            ...
            acc, mp, mr, mf1 = log_metrics_block(
                "LinearSVM",
                manip_name,
                lvl,
                yte_m,
            ...
            if COMPUTE_FEATURE_IMPORTANCE:
                ...
                used_names = (
                    sensors[:min(MAX_FI_FEATURES, len(sensors))]
                    if MAX_FI_FEATURES is not None
                    else sensors
                )

                add_feature_importance_rows(
                    src,
                    building,
                    LABEL_MODE,
                    tag_base,
            ...
            if USE_XGBOOST and "XGBoost" in trained:
                ...
                if COMPUTE_FEATURE_IMPORTANCE:
                    mean_imp, std_imp = sage_importance_tabular(
                        predict_fn=lambda Z: le.inverse_transform(
                            coerce_pred_labels(xgb_model.predict(Z))
                        ),
                        X_test=Xte_m,
                        y_test=yte_m,
                        X_train=X_train_for_fi_tab,
